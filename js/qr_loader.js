const showQrLoadError = (error) => {
  console.error('Lunch Check QR script load failed', error);
  ...
  msg.textContent = 'QR 화면 스크립트를 불러오지 못했습니다. 서버의 js 폴더 배포 상태를 확인해 주세요.';
  document.body.appendChild(msg);
};

(async () => {
  try {
    await import('./qr_app.js?v=20260702-final-refactor');
  } catch (error) {
    showQrLoadError(error);
  }
})();
